...
export default class Server {
	...
	private handleNewConnection(socket: Socket): void {
		...
		this.connections.set(connectionUuid, socketConnection);
	}
	...
	public sendMessageToUser(destinatoryName: string, message: Message): void {
		// Send message to user --> search for user connection & send message
		const userConnection = this.getUserConnectionByName(destinatoryName);
		if (userConnection) userConnection.sendMessage(message);
		// Add message to the user Queue
		const userQueue = this.userMessages.get(destinatoryName);
		if (userQueue) {
			// Push the incoming message to the user's queue
			userQueue.push(message);
		} else {
			// Creates a new Queue instance
			const newQueue = this.createInboundMessagesQueue(destinatoryName);
	...
	public createInboundMessagesQueue(userName: string) {
		...
		this.userMessages.set(userName, newQueue);
		return newQueue;
	}
}